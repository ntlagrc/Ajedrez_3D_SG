...
class Farola extends THREE.Object3D {
  ...
  constructor(material) {
    ...
    if (Farola.geom === null) {
      Farola.geom = this.buildGeometry();
    }

    const mesh = new THREE.Mesh(Farola.geom, material);
  ...
  buildGeometry() {
        ...
        var cabina =  evaluador.evaluate(posteBrush, cabina2, CSG.ADDITION);
        var farola = evaluador.evaluate(cabina, topFarolaBrush, CSG.ADDITION);
        farola.geometry.rotateY(Math.PI/4);
    
    const finalGeom = farola.geometry.clone();
