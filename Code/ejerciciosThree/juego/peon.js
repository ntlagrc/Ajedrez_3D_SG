...
class Peon extends THREE.Object3D  {
  ...
  constructor(material) {
    ...
    if (Peon.geom === null) {
      Peon.geom = this.buildGeometry();
    }

    const mesh = new THREE.Mesh(Peon.geom, material);
  ...
  getValidMoves(boardState) {
    ...
    const r1 = this.row + dir, c1 = this.col;
    if (isEmpty(boardState, r1, c1)) {
      moves.push({ row: r1, col: c1 });
      const r2 = this.row + 2 * dir;
      if (this.row === startRow && isEmpty(boardState, r2, c1)) {
        moves.push({ row: r2, col: c1 });
      }
    }

    for (const dc of [-1, +1]) {
      const rc = this.row + dir, cc = this.col + dc;
      if (isEnemy(boardState, rc, cc, this.color)) {
  ...
  buildGeometry() {
    ...
    var ojosNBrush = new CSG.Brush(ojosN,material_rojo);
    var ojosIzqBrush = new CSG.Brush(ojoIzq,material_rojo);
    //cuerpo
    var cuerpoBaseBrush = new CSG.Brush(cuerpoBase,material_rojo);
    var claviculasBrush = new CSG.Brush(claviculas,material_rojo);
    var araniaBrush = new CSG.Brush(arania,material_rojo);
    var recAraniaBrush = new CSG.Brush(recArania,material_rojo);
    ...
    const finalGeom = peon.geometry.clone();
