...
class Rey extends THREE.Object3D {
  ...
  constructor(material) {
    super();
    if (Rey.geom === null) {
      Rey.geom = this.buildGeometry();
    }
    const mesh = new THREE.Mesh(Rey.geom, material);
  ...
  getValidMoves(boardState) {
    ...
    for (const [dr,dc] of dirs) {
      ...
      if (inBounds(r,c) && (isEmpty(boardState,r,c) || isEnemy(boardState,r,c,this.color))) {
...
;
    ...
    var cuernoBaseBrush = new CSG.Brush(cuernoBase,material_rojo);
    var cuernoRecorteBrush = new CSG.Brush(cuernoRecorte,material_rojo);
    ...
    var recCapaDelanteBrush = new CSG.Brush(recCapaDelante,material_rojo);
    ...
    var baseCetroBrush = new CSG.Brush(baseCetro,material_rojo);
    var recCetroBrush = new CSG.Brush(recCetro,material_rojo);
    var piedraBrush = new CSG.Brush(piedra,material_rojo);
    ...
    const finalGeom = rey.geometry.clone();
    return finalGeom;
  }
